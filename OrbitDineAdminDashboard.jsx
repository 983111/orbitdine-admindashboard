// OrbitDine Super Admin Dashboard
// Built with React + Supabase MCP via Anthropic API
...
// ─── Stat Card ────────────────────────────────────────────────────────────────
function StatCard({ label, value, sub, icon, color = "orange", sparkData, trend }) {
  const colors = {
...
function Toasts({ toasts }) {
  const colors = { success: "border-emerald-500/30 text-emerald-400", error: "border-red-500/30 text-red-400", info: "border-indigo-500/30 text-indigo-400" };
  return (
...
function Restaurants({ toast }) {
  ...
  const toggleOperational = async (r) => {
    try {
                    ...
                    <div className="space-y-1.5">
                      {detail.feedback.slice(0, 5).map((f) => (
                        <div key={f.id} className="px-3 py-2 bg-white/[0.03] rounded-lg text-xs space-y-1">
                          ...
                          {f.comment && <p className="text-[#8080A0] italic">"{f.comment}"</p>}
                        </div>
                      ))}
                    </div>
                  </div>
    ...
    <div className="space-y-5 animate-fade-up">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className={cn("glass rounded-xl p-3.5 border", s.color === "amber" ? "border-amber-500/20" : s.color === "indigo" ? "border-indigo-500/20" : s.color === "green" ? "border-emerald-500/20" : "border-orange-500/20")}>
            <p className="text-xs text-[#6B6B9A]">{s.label}</p>
            <p className={cn("text-xl font-bold mt-0.5", s.color === "amber" ? "text-amber-400" : s.color === "indigo" ? "text-indigo-400" : s.color === "green" ? "text-emerald-400" : "text-orange-400")}>{s.value}</p>
          </div>
        ))}
      </div>
        ...
        <div className="flex rounded-xl overflow-hidden border border-white/[0.08]">
          {["7", "30", "90"].map((p) => (
            <button key={p} onClick={() => setPeriod(p)}
              className={cn("px-4 py-2 text-xs font-semibold transition-all", period === p ? "bg-orange-500 text-white" : "bg-white/[0.03] text-[#6B6B9A] hover:bg-white/[0.07]")}>
              {p === "7" ? "7D" : p === "30" ? "30D" : "90D"}
            </button>
          ))}
        </div>
...
function FeedbackDashboard({ toast }) {
  ...
  const low = feedback.filter((f) => f.rating <= 2);
  const filtered = feedback.filter((f) => filter === "all" || String(f.rating) === filter || (filter === "low" && f.rating <= 2) || (filter === "unresponded" && !f.manager_response));
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Negative (1-2★)" value={fmtNum(low.length)} sub="needs attention" icon={icons.alerts} color="rose" />
      ...
      <div className="space-y-2 overflow-y-auto" style={{ maxHeight: "50vh" }}>
        {filtered.map((f) => (
          <div key={f.id} className="glass rounded-xl p-4 space-y-2">
            ...
            {f.comment && <p className="text-sm text-[#C0C0E0] italic">"{f.comment}"</p>}
            {f.manager_response && (
...
function Alerts({ toast }) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      setAlerts(found.sort((a, b) => ({ high: 3, medium: 2, low: 1 }[b.severity] - { high: 3, medium: 2, low: 1 }[a.severity])));
    } catch (e) { toast(e.message, "error"); }
    setLoading(false);
  }, []);
        ...
        {alerts.map((a, i) => {
          ...
          return (
            <div key={i} className={cn("glass rounded-xl p-4 flex items-start gap-3 border",
              ...
              <div className={cn("w-2 h-2 rounded-full mt-1.5 shrink-0", a.severity === "high" ? "bg-rose-400" : a.severity === "medium" ? "bg-amber-400" : "bg-indigo-400")} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="glass rounded-2xl p-5 space-y-3 border border-amber-500/10">
            ...
            <div className="space-y-2">
              {[
                { label: "Refresh All Data", desc: "Pull latest from Supabase", action: () => { load(); toast("Data refreshed", "success"); } },
                { label: "Export Restaurant List", desc: "Download as JSON", action: () => { const blob = new Blob([JSON.stringify(restaurants, null, 2)], { type: "application/json" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "orbitdine_restaurants.json"; a.click(); toast("Exported!", "success"); } },
              ].map((a) => (
                <button key={a.label} onClick={a.action}
                  ...
                  <div>
                    <p className="text-sm font-semibold text-[#F0F0FF]">{a.label}</p>
                    <p className="text-xs text-[#6B6B9A]">{a.desc}</p>
                  </div>
...
export default function App() {
  ...
  const SECTIONS = {
    dashboard: <Dashboard />,
    restaurants: <Restaurants toast={toast} />,
    orders: <OrdersMonitor toast={toast} />,
    payments: <Payments toast={toast} />,
    feedback: <FeedbackDashboard toast={toast} />,
    qr: <QRHealth toast={toast} />,
    alerts: <Alerts toast={toast} />,
    controls: <AdminControls toast={toast} />,
  };
